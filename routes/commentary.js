...
module.exports = [
    ...
    },
    {
        ...
        handler: async (request, h) => {
            ...
            if(!request.payload 
                ...
                || !request.payload.idUser){
                response.error = "You must at least give the text, idUser and idPost.";
                return h.response(response).code(401);
            }
            
            var values; 
            var text = "INSERT INTO public.commentary(text, datecreation, individual_idindividual, post_idpost) VALUES ($1, $2, $3, $4);"
            values = [
                request.payload.text, 
                utils.formatDate(), 
                request.payload.idUser,
    ...
    {
        method: 'GET',
        path: '/getCommentaryListByIdPost',
        handler: async (request, h) => {
            response.error = null;
            response.body = [];
            //the request doesn't have all the information for getting a commentary from the database
            if(!request.query || !request.query.idPost){
                response.error = "You must at least give the id post.";
                return h.response(response).code(401);
            }
            
            var values;

            var text = "SELECT * FROM public.commentary WHERE post_idpost = $1 ORDER BY datecreation ASC;";
            values = [
